// Integration code for the Julia set fractal scene with Three.js
...
export class FractalScene {
  ...
  createShaderMaterial() {
    // Load shader code
    Promise.all([
      fetch('julia_vertex.glsl').then(response => response.text()),
      fetch('julia_fragment.glsl').then(response => response.text())
  ...
  setupBackgroundMusic() {
    ...
    this.audioElement.src = 'assets/music3.mp3';
  ...
  updateFormulaValues() {
    ...
    if (window.formulaController && this.uniforms && this.uniforms.u_mouse) {
      ...
      const juliaElement = document.getElementById('julia-c');
      
      if (juliaElement) {
        const real = (-0.8 + mouseX * 1.6).toFixed(2);
        const imag = (-0.8 + mouseY * 1.6).toFixed(2);
        const sign = imag >= 0 ? '+' : '';
        juliaElement.textContent = `${real} ${sign} ${imag}i`;
      }
    }
  }
  ...
  addInfoText() {
    ...
    infoElement.style.color = '#ff00ff';
    ...
    infoElement.textContent = 'Move mouse to change Julia set parameters';
