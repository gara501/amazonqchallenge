// Integration code for the Mandelbrot set fractal scene with Three.js

import * as THREE from 'three';
import { getAudioController } from './audio-controller.js';

export class MandelbrotScene {
  constructor() {
  ...
  createShaderMaterial() {
    // Load shader code
    Promise.all([
      fetch('mandelbrot_vertex.glsl').then(response => response.text()),
      fetch('mandelbrot_fragment.glsl').then(response => response.text())
  ...
  setupBackgroundMusic() {
    ...
    this.audioElement.src = 'assets/music4.mp3';
  ...
  updateFormulaValues() {
    ...
    if (window.formulaController && this.uniforms && this.uniforms.u_mouse) {
      ...
      // Find the formula value element
      const offsetElement = document.getElementById('mandelbrot-offset');
      
      if (offsetElement) {
        const offsetX = (mouseX * 2 - 1).toFixed(2);
        const offsetY = (mouseY * 2 - 1).toFixed(2);
        offsetElement.textContent = `(${offsetX}, ${offsetY})`;
      }
    }
  }
  ...
  addInfoText() {
    ...
    infoElement.style.color = '#00ff88';
    ...
    infoElement.textContent = 'Move mouse to explore the Mandelbrot set';
