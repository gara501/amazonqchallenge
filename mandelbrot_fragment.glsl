// Mandelbrot set fractal fragment shader
...
vec3 mandelbrot(vec2 c, vec2 offset, float zoom) {
    // Initialize z to 0
    vec2 z = vec2(0.0);
    
    // Iterate the Mandelbrot set formula: z = z^2 + c
    int iterations = 0;
...
void main() {
    ...
    vec3 color = mandelbrot(rotatedC, offset, zoom);
