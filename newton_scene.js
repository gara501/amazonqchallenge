// Integration code for the Newton-Raphson fractal scene with Three.js

import * as THREE from 'three';
import { getAudioController } from './audio-controller.js';

export class NewtonScene {
  constructor() {
  ...
  createShaderMaterial() {
    // Load shader code
    Promise.all([
      fetch('newton_vertex.glsl').then(response => response.text()),
      fetch('newton_fragment.glsl').then(response => response.text())
  ...
  setupBackgroundMusic() {
    ...
    this.audioElement.src = 'assets/music5.mp3';
  ...
  updateFormulaValues() {
    ...
    if (window.formulaController && this.uniforms && this.uniforms.u_mouse) {
      ...
      const degreeElement = document.getElementById('newton-degree');
      const relaxationElement = document.getElementById('newton-relaxation');
      
      if (degreeElement) {
        const degree = Math.floor(3 + mouseX * 3);
        degreeElement.textContent = degree;
      }
      
      if (relaxationElement) {
        const relaxation = (0.5 + mouseY * 1.0).toFixed(2);
        relaxationElement.textContent = relaxation;
      }
    }
  }
  ...
  addInfoText() {
    ...
    infoElement.textContent = 'Move mouse: X = polynomial degree, Y = relaxation factor';
